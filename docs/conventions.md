...
- `strategy.*`(rungs/fractions/anchor_ema_span/rebuy_offset) 驱动推荐策略 **以及 paper/live 引擎**（paper == backtest，规则同源）；`baseline.*` 驱动原始 freqtrade 策略；`backtest.*` 驱动 alloc/adv 扫描/fill 模型；`sweep.*` 驱动参数扫描；`dryrun.*` 驱动实测工具；`live.*`(mode/confirm_env/api_key_env/api_secret_env/max_order_usd) 驱动 paper/live 引擎的安全闸；`universe`/`market` 全局共用。
...
- 入口统一走 `sca.cli`（用 runpy 跑各模块的 `__main__`，行为等同 `python -m sca.<module>`）。`paper` 与 `live` 都映射到 `sca.live.engine`，靠 `--mode` 区分（默认 paper）。
...
- 上实盘前：服务器跑 paper 引擎（`docker compose up -d --build` 起 paper + dashboard）看真实成交质量（ROUND-TRIP markout）与仓位演化，再决定。
- paper 零下单零密钥；live 模式已搭脚手架但被三重闸严控，需显式授权 + confirm + API key 才会真实下单。

...
Dockerfile · docker-compose.yml (default: bot + dashboard; profiles: tools) · docker-entrypoint.sh
```
...
```bash
docker compose up -d --build                   # starts bot + dashboard → http://<host>:3015
docker compose logs -f bot                     # watch fills + markout; status JSON + CSV in ./out
```
